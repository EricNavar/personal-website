...
import { ContentfulExperienceItem, ContentfulResumePage, ContentfulSkill, ExperienceItem, Skill } from '../commonTypes';
...
function ResumePage(): JSX.Element {
  ...
  const [studentOrgInvolvement, setStudentOrgInvolvement] = React.useState<ExperienceItem[]>([]);
  React.useEffect(() => {
    client.getEntries({
      ...
      .then((response) => {
        ...
        const experienceData = resumePage.internshipExperience.map((experience: ContentfulExperienceItem) => {
          return {
            organization: experience.fields.organization,
            organizationIcon: experience.fields.organizationIcon.fields.file.url,
        ...
        const studentOrgInvolvementData = resumePage.studentOrgInvolvement.map((involvement: ContentfulExperienceItem) => {
          return {
            organization: involvement.fields.organization,
            organizationIcon: involvement.fields.organizationIcon.fields.file.url,
  ...
  return (
    <ScreenMain>
      ...
      <InvolvementSection title="Student Org Involvement" data={studentOrgInvolvement} />
      <ScreenBackground />
    </ScreenMain>
  );
}
